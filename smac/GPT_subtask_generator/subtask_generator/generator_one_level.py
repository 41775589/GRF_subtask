...
SMAC_MEDoE_DIR = os.path.abspath(os.path.join(current_dir, "../../../"))

SRC_DIR = os.path.join(SMAC_MEDoE_DIR, "doe_epymarl-main/src")
...
SMAC_DIR = os.path.dirname(parent_dir)
CONFIG_ROOT_DIR = os.path.join(SMAC_MEDoE_DIR, 'doe_epymarl-main/src/config/envs')
MAP_DIR = os.path.join(SMAC_MEDoE_DIR, 'doe_epymarl-main/src/envs/smac/maps')
REWARD_DIR = os.path.join(SMAC_MEDoE_DIR, 'doe_epymarl-main/src/envs/smac/rewards')
...
def main(model, n_decomposition, n_reward, temperature, task, alg_cfg, use_doe, n_improve_iter):
    ...
    env = f'{ROOT_DIR}/env_code/starcraft2.py'
    env_code = file_to_string(env)
    map_registry= f'{ROOT_DIR}/env_code/smac_maps.py'
    map_registry = file_to_string(map_registry)
    ...
    # Scenario Need to Consider: Use predefined map?
    ...
    main_task = "learn to play in the 8m map: n_agents:8, n_enemies:8, limit:120, a_race:T, b_race:T, unit_type_bits:0, map_type: marines "
    num_agents = 8
    num_groups = 2
    ...
    for response_id in range(n_decomposition):
        ...
        for group_id in range(len(groups)):
            ...
            cur_initial_system_scenarios = initial_system_scenarios
            ...
            with open(f"{SMAC_DIR}/scenarios/{Time}/scenario_layer0_decomposition{response_id}_subtask{group_id}.py",
                      'w') as file:
                file.writelines("from . import *" + '\n')
                file.writelines(scenario_code_string + '\n')
            ...
            curr_code_output_tip_rewards = code_output_tip_rewards
            cur_initial_system_rewards = initial_system_rewards  + curr_code_output_tip_rewards
            ...
            for i in range(n_improve_iter):
                ...
                for response_r_id in range(n_reward):
                    ...
                    with open(
                            f"{SMAC_DIR}/rewards/reward_layer0_decomposition{response_id}_subtask{group_id}_sample{response_r_id}.py",
                            'w') as file:
                        file.writelines(reward_code_string + '\n')
...
if __name__ == "__main__":
    main(model="gpt-3.5-turbo", n_decomposition=1, n_reward=1, temperature=1, task="smac", alg_cfg="ia2c",
